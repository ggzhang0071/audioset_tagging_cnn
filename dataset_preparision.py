...
def  collect_data_from_ef(wav_dir,save_dir,save_test_json):
    wav_file_list=[]
    counter=0
    for wav_file in iglob(wav_dir+"/*.wav"):
            wav_file_list.append({"wav":wav_file})  
            counter+=1
            if counter>=200:
                break
    print("The total type ef_audio num is {}".format(len(tuple(wav_file_list))))

    with open(os.path.join(save_dir,save_test_json), 'w') as f:
        json.dump({"data":wav_file_list}, f,indent=4)
        print("Finish the ef_audio dataset preparation")
    ...
    for parent_name,types in choosed_types.items():    
        ...
        for index_name_pair in types:
            ...
            for csv_file in iglob(csv_path):
                ...
                wav_names=df.iloc[:,0]+"_"+df.iloc[:,1].astype(int).astype(str) +".000"
            ...
            for wav_file in iglob(wav_path+"/*.wav"):
                ...
                for i, wav_name_ in enumerate(wav_names):
                    ...
                    if wav_name==wav_name_:
                        ...
                        counter+=1
                        break
            print("The type {} num is {}".format(file1,counter))
        parent_label+=1
...
if __name__=="__main__":
    ...
    if args.dataset=="audioset":
        ...
        original_choosed_types=['Baby cry, infant cry',  'Baby laughter' , 'Child singing',  'Children shouting', 'Children playing', 'Child speech, kid speaking', 'Female singing', 'Female speech, woman speaking','Male singing', 'Male speech, man speaking']
            ...
            for type1 in  reversed(classification_types):
                for sub_type in reversed(original_choosed_types):
                    if type1 in sub_type:
                        ...
                        original_choosed_types.remove(sub_type)
    ...
    if test_dataset=="esc50":
        ...
        save_test_json="ef_audio_data.json"
        collect_data_from_ef(args.test_wav_dir,save_dir,save_test_json)
